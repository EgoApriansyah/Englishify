<x-guest-layout>
    ...
    <form method="POST" action="{{ route('register') }}" class="space-y-5">
        ...
        <!-- Name -->
        <div>
            <label for="name" class="block text-label text-muted uppercase tracking-widest mb-2 font-body font-semibold">Nama Lengkap</label>
            <div class="relative">
                ...
                <input id="name" type="text" name="name" value="{{ old('name') }}" required autofocus autocomplete="name" 
                    class="block w-full pl-11 pr-4 py-3 bg-canvas border border-hairline text-ink placeholder-muted rounded-sm focus:border-green focus:ring-4 focus:ring-green/15 focus:outline-none transition duration-150 text-body-md font-medium" placeholder="Nama Lengkap Anda" />
            </div>
        ...
        <div x-data="{ showPassword: false }">
            <label for="password" class="block text-label text-muted uppercase tracking-widest mb-2 font-body font-semibold">Kata Sandi</label>
            <div class="relative">
                <div class="absolute inset-y-0 left-0 pl-3.5 flex items-center pointer-events-none text-muted">
                    <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-5 h-5">
                ...
                <input id="password" :type="showPassword ? 'text' : 'password'" name="password" required autocomplete="new-password" 
                    class="block w-full pl-11 pr-11 py-3 bg-canvas border border-hairline text-ink placeholder-muted rounded-sm focus:border-green focus:ring-4 focus:ring-green/15 focus:outline-none transition duration-150 text-body-md font-medium" placeholder="Buat kata sandi minimal 8 karakter" />
        ...
        <div x-data="{ showConfirmPassword: false }">
            <label for="password_confirmation" class="block text-label text-muted uppercase tracking-widest mb-2 font-body font-semibold">Konfirmasi Kata Sandi</label>
            <div class="relative">
                <div class="absolute inset-y-0 left-0 pl-3.5 flex items-center pointer-events-none text-muted">
                    <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-5 h-5">
                      <path stroke-linecap="round" stroke-linejoin="round" d="M16.5 10.5V6.75a4.5 4.5 0 1 0-9 0V10.5m-2.25 13.5h13.5c1.242 0 2.25-1.008 2.25-2.25V11.25c0-1.242-1.008-2.25-2.25-2.25H5.25c-1.242 0-2.25 1.008-2.25 2.25v11.25c0 1.242 1.008 2.25 2.25 2.25Z" />
                ...
                <input id="password_confirmation" :type="showConfirmPassword ? 'text' : 'password'" name="password_confirmation" required autocomplete="new-password" 
                    class="block w-full pl-11 pr-11 py-3 bg-canvas border border-hairline text-ink placeholder-muted rounded-sm focus:border-green focus:ring-4 focus:ring-green/15 focus:outline-none transition duration-150 text-body-md font-medium" placeholder="Ulangi kata sandi Anda" />
                <button type="button" @click="showConfirmPassword = !showConfirmPassword" class="absolute inset-y-0 right-0 pr-3.5 flex items-center text-muted hover:text-ink transition-colors">
        ...
        <div class="text-center mt-6 pt-4 border-t border-hairline">
            <p class="text-body-sm text-muted font-semibold">
                Sudah punya akun? 
                <a href="{{ route('login') }}" class="font-bold text-green hover:text-green-dark transition-colors">
                    Masuk
                </a>
            </p>
        </div>
